...
def signup(request, slug):
    ...
    # FIXME: refactore this with Django forms
    skills = [ k.split("_")[1] for k, v in body.items() if k.startswith("skill_") and v == "on"]
    attendance = [ k.split("_")[1] for k, v in body.items() if k.startswith("attendance_") and v == "on"]
    teams = [ k.split("_")[1] for k, v in body.items() if k.startswith("team_") and v == "on"]

    event = Event.objects.get(slug=slug)
    crew = Crew.objects.get(event=event)

    try:
        Person.objects.get(email=body.get("person_email"), last_name=body.get("person_lastname"))
        ...
        crew_member = CrewMember.objects.create(
            person=person,
            birthday=make_aware(datetime.strptime(body.get("person_birthday"), "%Y-%m-%d")),
            crew=crew,
...
def request_magic_link(request):
    try:
        body = json.loads(request.body)
        person = Person.objects.get(email=body.get("contact_email"), last_name=body.get("person_lastname"))
        ...
        magic_link = MagicLink.objects.create(person=person, expires_at=datetime.now() + timedelta(weeks=4))
        magic_link.save()
    ...
    return JsonResponse({"status": "ok", "message": "Magic link sent if mail and lastname match"})
